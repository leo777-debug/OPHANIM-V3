...
export async function syncExistingOphanimDisruptions(origin: string, actor?: OrganizationActor): Promise<{ fetched: number; upserted: number; reconciled: number; errors: string[] }> {
  ...
  const owners = actor
    ? [{ organization_id: actor.organizationId, user_id: actor.userId }]
    : (await db().query<OrganizationOwner>(
  ...
  for (const owner of owners) {
    for (const item of normalized) {
      try {
        ...
        upserted++;
        await reconcileDisruptionImpacts(actor ?? { organizationId: owner.organization_id, userId: owner.user_id, role: 'owner' }, disruption.id);
        reconciled++;
      } catch (error) {
